...
const app = express();
// Версия сборки для cache-busting (обновляется при деплое)
const BUILD_VERSION = process.env.BUILD_VERSION || process.env.RENDER_GIT_COMMIT || String(Date.now());
const PORT = process.env.PORT || 3000;
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || '';
const KEEP_ALIVE = process.env.KEEP_ALIVE === '1' || process.env.KEEP_ALIVE === 'true';
...
// Статические файлы
// Статика: CSS/JS/шрифты можно кэшировать дольше, HTML — нельзя
app.use((req, res, next) => {
    // Не кэшируем HTML документы, чтобы мобильные устройства всегда получали актуальный index.html
...
app.use(express.static(path.join(__dirname), {
    // Дадим умеренный кэш для ассетов. Версионирование через ?v=BUILD_VERSION принудит обновление.
    maxAge: '7d',
    etag: true
}));
...
// Вспомогательная функция: отдать HTML с автоматической подстановкой версии для ассетов
function sendVersionedHtml(res, filePath) {
    try {
        let html = fs.readFileSync(filePath, 'utf8');
        // Подменяем версии в ссылках на ассеты: styles.css, script.js
        html = html
            .replace(/styles\.css\?v=[^"']*/g, `styles.css?v=${BUILD_VERSION}`)
            .replace(/script\.js\?v=[^"']*/g, `script.js?v=${BUILD_VERSION}`)
            // На всякий случай — если параметра нет, добавим
            .replace(/styles\.css(\"|\')/g, `styles.css?v=${BUILD_VERSION}$1`)
            .replace(/script\.js(\"|\')/g, `script.js?v=${BUILD_VERSION}$1`);
...
// Простая админ-авторизация по заголовку
// Авторизация отключена по запросу — middleware пропускает всех
...
// Главная страница
app.get('/', (req, res) => {
    return sendVersionedHtml(res, path.join(__dirname, 'index.html'));
});

// Диагностическая страница
app.get('/debug', (req, res) => {
    return sendVersionedHtml(res, path.join(__dirname, 'debug.html'));
});

// Админ-панель
app.get('/admin', (req, res) => {
    return sendVersionedHtml(res, path.join(__dirname, 'admin.html'));
});
...
app.delete('/api/delete-image/:id', verifyAdmin, async (req, res) => {
    try {
        if (!SUPABASE_CONFIG.client) {
...
app.listen(PORT, async () => {
    ...
    // Self-ping keep-alive (по желанию через переменную окружения KEEP_ALIVE)
    if (KEEP_ALIVE) {
        const url = process.env.SITE_URL || `http://localhost:${PORT}`;
        setInterval(() => {
            fetch(`${url}/health`).catch(() => {});
        }, 10 * 60 * 1000);
        console.log('🟢 Keep-alive включен');
    }
});
