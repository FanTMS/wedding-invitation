...
function updateSiteDisplay() {
    ...
    if (weddingTextEl && !isNaN(date.getTime())) {
        // Оставляем только текст без даты
        weddingTextEl.textContent = 'Приглашаем вас';
    }
...
function updateOtherImages() {
    ...
    if (SITE_CONFIG.images.zagsPhoto) {
        ...
        const zags = document.getElementById('zags-img');
        if (zags) zags.removeAttribute('src');
    }
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Keep-alive для Render: мягкий пинг раз в 10 минут, чтобы не засыпал инстанс
    try {
        setInterval(() => {
            fetch('/health').catch(() => {});
        }, 10 * 60 * 1000);
